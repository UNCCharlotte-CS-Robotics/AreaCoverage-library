 ...
 */
#include <aclibrary/video_generator.h>
#include <cmath>

namespace aclibrary {
	typedef lclibrary::Vec2d Vec2d;
	...
	void VideoGenerator::GnuplotMap(const std::string &output_plot_file_name, bool curr_req, bool req, bool non_req,bool at_end) {
		...
		out_file << "plot ";
		out_file << "\"" << outer_polygon_filename_ << "\" u ($1/"<<scale_<<"):($2/"<<scale_ <<") with filledcurves closed lw 2 lc rgbcolor \"gray40\" fill transparent solid 0,";
		if(has_holes_ > 0) {
			for(size_t ii = 0; ii < num_holes_; ++ii) {
				out_file << "\"" << holes_filename_ + std::to_string(ii) << "\" u ($1/"<<scale_<<"):($2/"<<scale_ <<") w filledcurves closed lw 2 lc rgbcolor \"gray40\" fill transparent solid 0.4,";
			}
		}
		if (req) {
